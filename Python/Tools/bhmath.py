...
class Set:
    ...
    def __init__(self, base:Union[SetTypes, set, list, Iterable, Self], conditions:Iterable[Callable[[Any], bool]] = [lambda x:True])->None:
        ...
        if type(base) == SetTypes:
            ...
            if isinstance(base._finite, set):
                ...
                self._conditions = [] # after all conditions are applied to the _finite internal set, they serve no more purpose
                self._generator = self._finite.__iter__
            else: # The base set is either a (sub)set of omega or uses a possibly infinite generator
                self._finite = base._finite
                self._conditions = base._conditions + conditions
                self._generator = self._Generator(base._generator, conditions)
...
_RealNumbers = Set(SetTypes.All, [lambda x:isinstance(x, Real)])
